...
app.post("/api/auth/register", authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (EMAIL_ENABLED) {
      await pool.query(
        "INSERT INTO users (id, email, full_name, password_hash, email_verified) VALUES ($1, $2, $3, $4, false)",
        [id, normalizedEmail, fullName.trim(), passwordHash]
      );
      const token = await createAuthToken(id, "verify_email", 24 * 60);
      const verifyUrl = `${getAppUrl(req)}/api/auth/verify-email?token=${token}`;
      const sent = await sendEmail(
        normalizedEmail,
        "Verify your email — QR Attendance System",
        `<p>Hi ${fullName.trim()},</p>
         <p>Confirm your email to activate your coordinator account:</p>
         <p><a href="${verifyUrl}">${verifyUrl}</a></p>
         <p>This link expires in 24 hours. If you didn't create this account, you can ignore this email.</p>`
      );
      if (!sent) {
        // Sending failed even though a provider is configured (e.g. quota,
        // bad key) - don't leave the account permanently unverifiable.
    ...
    // No email provider configured - fall back to the original, immediate-login
    // behavior so local development isn't blocked on setting up email.
    await pool.query(
...
// Email verification link handler (GET, since it's opened directly from an
// email client, not called via fetch). Redirects back to the login page with
// a query flag the frontend reads to show a success/failure banner.
