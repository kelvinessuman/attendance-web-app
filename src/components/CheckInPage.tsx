...
export default function CheckInPage({ groupId }: CheckInPageProps) {
  ...
  // 1. Fetch Active Session details on mount
  ...
  // Re-poll the server periodically so the page notices when a session truly
  // expires (or a new one starts) without the student having to hit refresh.
  useEffect(() => {
    const pollId = setInterval(() => {
      // Don't interrupt someone mid check-in with a re-fetch/camera reset.
